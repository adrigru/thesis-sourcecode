...
class Classifier(nn.Module):
    def __init__(self, embeddings_size, n_classes):
        ...
        self.fc1 = nn.Linear(self.embeddings_size, self.n_classes)
...
#
#     f, axes = plt.subplots(1, _k, figsize=(20, 8), )
#
...
def get_data(data_dir, filename):
    ...
    tensor_dataset = torch.load(os.path.join(data_dir, filename))
...
def train_model(data_x, labels, batch_size, epochs, learning_rate):
    ...
    X_train, X_test, y_train, y_test = train_test_split(data_x, labels, test_size=.1, random_state=12345, shuffle=True, stratify=labels)
    X_train, X_val, y_train, y_val = train_test_split(X_train, y_train, random_state=69420, shuffle=True, stratify=y_train)


    train_data, _ = init_dataloaders(X_train, y_train, X_val, y_val, batch_size=batch_size)
    # train_data, _ = init_dataloaders(X_train, y_train, X_test, y_test, batch_size=batch_size)
    class_weights = get_class_weights(y_train)
    ...
    metrics = fit(model, criterion, train_data, None, epochs, learning_rate)
    history.append(metrics)

    logger.info(f"Best QWK for each fold: {np.max([metric['train_qwk'] for metric in history], axis=1)} in {np.argmax([metric['train_qwk'] for metric in history], axis=1)}")
    # logger.info(f"Best F1-Micro each fold: {np.max([metric['f1_micro'] for metric in history], axis=1)}")
    # logger.info(f"Best F1-Macro each fold: {np.max([metric['f1_macro'] for metric in history], axis=1)}")
    # logger.info(f"Best F1-Weighted each fold: {np.max([metric['f1_weighted'] for metric in history], axis=1)}")
...
if __name__ == "__main__":
    ...
    logger.info(
        ...
        f' reduced_classes={_learning_rate},'
        f' k={_k},'
        f' t_0={t_0},'
        f' t_mult={t_mult}'
    )
